...
def get_source(args):
    ...
    if args.video is None:
        ...
        cam = cv2.VideoCapture('falling.mp4')
        if isinstance(args.video, str):
            vid = [int(s) for s in re.findall(r'\d+', args.video)]
            if len(vid) == 5:
                tagged_df = pd.read_csv("Fall_Detection_Model/dataset/CompleteDataSet.csv", usecols=[
...
# Loading the model to show the prediction

def alg2_sequential(queues, argss, consecutive_frames, event, args1=None):
